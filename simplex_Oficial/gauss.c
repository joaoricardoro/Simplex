...
// Localização da variável de custo mínimo.  percorre. ponteiro para sobreviver ao fechamento da funcao.
...
// Eliminação de Gauss
void gauss(float matriz[TAMANHO][TAMANHO], float vetor_b[TAMANHO], float vetor_x[TAMANHO], int num_linhas, int num_colunas){ // num_colunas da matriz A !
	int i, j, k;
	float subtrativo;
	...
	for(j = 0; j < num_colunas + 1; j++){
		for(i = j + 1; i < num_linhas; i++){
			...
			if(matriz[j][j] == 0){
                printf("\n\nEste sistema possui infinitas solucoes.\n\n");
			}
			else{
...
void simplex(float matriz_A[TAMANHO][TAMANHO], int vetor_relacional[TAMANHO], float vetor_b[TAMANHO], int minimiza, float vetor_custo[TAMANHO], float vetor_x[TAMANHO], float *valor_Z, int num_linhas, int num_colunas){
    ...
    int vetor_variaveis[TAMANHO]; // vetor com a coluna da variável na matriz padrodizada.
    int vetor_base[TAMANHO];

    // Ajuste dos b[i] negativos. Troca sinal de tudo.
    for(i = 0; i < num_linhas; i++){
        if(vetor_b[i] < 0){
            for(j = 0; j < num_colunas; j++){
    ...
    for(i = 0; i < num_linhas; i++){
        ...
        if(vetor_relacional[i] == -1){ // menor ou igual que.
            ...
            if(vetor_relacional[i] == 1){ // maior ou igual que.
                ...
                if(vetor_relacional[i] == 0){ // ESTA PARTE MERECE MAIOR ATENÇÃO NO FUTURO.
                    matriz_padronizada[i][num_colunas_mp] = 1;
                    vetor_base[i] = num_colunas_mp;
                    indica_fase1 = 1;
                }
    ...
    if(indica_fase1 == 1){
        ...
        while(custo_minimo < 0 && num_colunas_mp > num_linhas + num_colunas){
            ...
            gauss(matriz_base, vetor_b, vetor_x_temp, num_linhas, num_linhas);
            printf("\n\nVetor x da base:\n");
            ...
            gauss(matriz_base_t, vetor_custo_base, vetor_lambda, num_linhas, num_linhas);
            printf("\n\nVetor lambda:\n");
            ...
            // Passo 4.
                ...
                gauss(matriz_base, vetor_colunar, vetor_y, num_linhas, num_linhas);
                printf("\n\nVetor y:\n");
                ...
                if(temp < num_linhas + num_colunas){
